...
export const ScrollBanner = ({ children }: ScrollBannerProps) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 h-20 left-0 right-0 z-30 shadow-lg transition-all duration-300 ease-in-out transform flex items-center ${
          isScrolled
            ? "translate-y-0 opacity-100"
            : "-translate-y-full opacity-0"
        }`}
        style={{
          backdropFilter: "blur(8px)", // not going to work in ie
          backgroundColor: "rgba(40, 94, 80, 0.95)",
        }}
      >
